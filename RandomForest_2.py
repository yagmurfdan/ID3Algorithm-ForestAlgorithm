...
X = veriseti.drop(['sirket'], axis=1)
y = veriseti['sirket']

# eğitim ve test veri setleri oluşturuldu
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size = 0.30, random_state = 42)

encoder = ce.OrdinalEncoder(cols=['ulke','yil', 'izleyici','begenmek','money','yapimci_sayisi','imdb'])   # sözel bilgiler sayısal bilgilere dönüştürüldü
X_train = encoder.fit_transform(X_train)
X_test = encoder.transform(X_test)

model = RandomForestClassifier(n_estimators=10,random_state=0) # sınıflandırıcı model oluşturuldu(n_estimators, ağaç sayısını tutuyor.) 

model.fit(X_train, y_train)  # model eğitim testi ile eğitilir

y_pred = model.predict(X_test) # model test veri ile test edilir.

print('10 Karar ağaçlı model doğruluk skoru :  {0:0.4f}'. format(accuracy_score(y_test, y_pred))) # başarı sonucu ekrana yazdırılır
